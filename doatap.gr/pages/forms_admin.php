...
<head>
    ...
    <title>ΔΟΑΤΑΠ - Admin - Αιτήσεις</title>
    <style>
    </style>
</head>
<body>
    ...
    <div class="forms" id="forms">
        <h3 style="text-align:center"><strong>Αιτήσεις που έχουν υποβληθεί</strong></h3><br /><br /><br />
            <div class="table">
        <?php
        ...
        $sql = "SELECT * FROM form WHERE final = 0";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
    // output data of each row
    echo"<div class='table'>";
    echo"<form method='post' action='../php/view_form_2.php'>";
    echo"<table>";
    echo"<tr><th>Αριθμός αίτησης</th><th>Ημερομηνία υποβολής</th><th>Κατάσταση</th><th>Στάδιο</th>";
    while($row = $result->fetch_assoc()) {
    echo "<tr><td> <input type='Submit' name='formid' value='" . $row["id"]. "'/> </td><td>" . $row["date"] . "</td><td>" . $row["status"] . "</td><td>" . $row["stage"] . "</td>";
    }
    echo "</table>";
    echo"</form>";
    } else { echo "0 results"; }
